...
def paper_auditor_agent(state: ResearchMindState) -> ResearchMindState:
    ...
    # Guard: if no paper path provided, skip gracefully
    if not own_paper_path:
        msg = "paper_auditor_agent: no own_paper_path provided — skipping audit."
        state = {
            **state,
            "formatting_issues": [],
            "missing_sections": [],
    ...
    # ------------------------------------------------------------------ #
    # Step 2 — Build the LLM prompt from audit findings
    # ------------------------------------------------------------------ #
    user_prompt = _build_llm_prompt(audit_result, own_paper_path)
...
def _build_llm_prompt(audit_result: Dict[str, Any], paper_path: str) -> str:
    """
    Build a detailed prompt for the LLM from the tool's structured audit result.
